...
rcls_flux_df <- rcls %>% select(DateTime, Year, DoY, Hour, NEE_PI, LE, H, TA, TS_PI_1, RH, USTAR, P_PI_F, PA, WS, SW_IN, LW_IN, radNet, SWC_PI_F) 
...
# set column names
colnames(rcls_flux_df) <- c("DateTime", "Year", "DoY", "Hour", "NEE", "LE", "H", "Tair", "Tsoil", "rH", "Ustar", "P", "PA", "U", "Rg", "FLDS", "radNet", "SWC", "VPD", "PRECTmms")
...
EddyProc.C <- sEddyProc$new('Rcls', rcls_flux_df, c("NEE", "LE", "H", "Tair", "Tsoil", "rH", "Ustar", "P", "PA", "U", "Rg", "FLDS", "radNet", "SWC", "VPD", "PRECTmms"))
...
EddyProc.C$sMDSGapFill('SWC', FillAll = FALSE)  
...
# Write output to CLM
##############################################################################
DirOut <- "C:/Users/Derek/Google Drive/RCrk/RC Flux Tower data/RC-Rls/gap-filled_monthly_netCDF/wSWC/"
tower_name <- "rcls"
...
for (m in setYearMon) {
  ...
  SWC  <- ncdf4::ncvar_def("SWC", "%", list(lon,lat,time), mv,
                          longname="Volumetric Soil Water Content", prec="double")
  
  
  #Create the output file
  ncnew <- ncdf4::nc_create(fileOutNcdf, list(LATIXY,LONGXY,FLDS,FSDS,PRECTmms,RH,PSRF,TBOT,WIND,ZBOT,FSH,EFLX_LH_TOT,NEE,GPP,Rnet, SWC))
  ...
  ncdf4::ncvar_put(ncnew, SWC, Data.mon$SWC)
  #add attributes
  ...
  ncdf4::ncatt_put(ncnew, SWC,"mode","time-dependent" ,prec=NA,verbose=FALSE,definemode=FALSE )  
...
write.csv(dataClm, "C:/Users/Derek/Google Drive/RCrk/RC Flux Tower data/RC-Rls/gap-filled_csv_rds/rcls_dataClm_wSWC.csv")

#save as RDS
saveRDS(dataClm, file = "C:/Users/Derek/Google Drive/RCrk/RC Flux Tower data/RC-Rls/gap-filled_csv_rds/rcls_dataClm_wSWC.rds")


# Open and plot netCDF file to check time conversion
#############################################################
nc <- nc_open("C:/Users/Derek/Google Drive/RCrk/RC Flux Tower data/RC-Rls/gap-filled_monthly_netCDF/wSWC/2016-04.nc")
nc_time <- ncvar_get(nc, "time")
nc_SWC <- ncvar_get(nc, "SWC")
nc_df <- data.frame(time=nc_time, SWC = nc_SWC)
ggplot(nc_df, aes(x=time, y=SWC)) + geom_point() + geom_line() + ggtitle("netCDF data: April 2016")
ggplot(rcls, aes(x=DateTime, y=SWC_PI_F)) + geom_point()


nc_Rnet <- ncvar_get(nc, "Rnet")
nc_df2 <- data.frame(time=nc_time, Rnet = nc_Rnet)
ggplot(nc_df2, aes(x=time, y=Rnet)) + geom_point() + geom_line() + ggtitle("netCDF data: April 2016")
